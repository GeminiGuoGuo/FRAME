...
- (void)showInView:(UIView *)view
{
    ...
    //修改弹出框的位置
    if ( ! hasModifiedFrame) {
        self.frame = CGRectMake((view.frame.size.width - self.frame.size.width )/2, (view.frame.size.height - self.frame.size.height) /2 - 80, self.frame.size.width, self.frame.size.height);
    }
